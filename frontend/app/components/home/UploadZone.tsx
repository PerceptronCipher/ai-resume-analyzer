...
import Button from '../shared/Button' // Keep your custom button import
...
export default function UploadZone() {
  ...
  const TOKEN_COST = 200; // Define how much a resume scan costs

  // 1. Capture the DBChores session token on load
  useEffect(() => {
  ...
  // 2. Intercept the analyze click to ask DBChores for tokens first
  const handleAnalyzeClick = (mode: 'file' | 'text') => {
    setAnalysisMode(mode)
    setIsAnalyzing(true)

    // Request DBChores to deduct tokens
    window.parent.postMessage(
      {
        type: 'REQUEST_DEDUCTION',
        amount: TOKEN_COST,
        tool: 'Bitkinart Resume',
      },
      '*'
    )
  }

  // 3. Listen for approval, run the backend, save history & handle refunds
  useEffect(() => {
    const handleMessage = async (event: MessageEvent) => {
      // Step A: DBChores approved the deduction! Let's run the AI.
      if (event.data?.type === 'DEDUCTION_APPROVED') {
        const baseUrl = 'https://api-resume-analyzer.buildoninc.org'
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(data.detail || 'Analysis failed. Please try a different format.')
          }
          ...
          // Step B: Send the successful output back to DBChores to save in History
          // We save it as a JSON string so it looks beautiful in the DBChores History Modal!
          window.parent.postMessage(
            {
              type: 'SAVE_HISTORY',
              title: `Resume Analysis (Score: ${data.ats_score}%)`,
          ...
          // Step C: THE SAFETY NET. If the backend fails, refund the user!
          window.parent.postMessage(
            {
              type: 'REQUEST_REFUND',
              amount: TOKEN_COST,
      ...
      // Step D: DBChores rejected the deduction (user is out of tokens)
      else if (event.data?.type === 'DEDUCTION_FAILED') {
        setIsAnalyzing(false)
        toast.error('Insufficient tokens in your DBChores wallet.')
      }
    }

    window.addEventListener('message', handleMessage)
    return () => window.removeEventListener('message', handleMessage)
  }, [analysisMode, file, pastedText, sessionToken])
          ...
          {!isCompleted ? (
            <>
              {(file || isPasteMode) && !isAnalyzing && (
                <button
                  onClick={resetAll}
                  className='absolute top-4 right-4 bg-white shadow-sm border border-slate-100 text-slate-400 rounded-full p-2 hover:text-rose-500 transition-all cursor-pointer z-10'
                >
                  <X size={18} />
                </button>
              )}
              ...
              {isPasteMode ? (
                <div className='w-full max-w-2xl'>
                  ...
                  />
                  <Button
                    onClick={() => handleAnalyzeClick('text')}
                    disabled={isAnalyzing || !pastedText.trim()}
                    className='rounded-xl px-12 py-4 bg-[#074799] cursor-pointer'
                  >
                    {isAnalyzing ? (
                ...
                >
                  <div 
                    ...
                    className='w-20 h-20 bg-blue-100 rounded-full flex items-center justify-center mb-6 cursor-pointer'
                  >
                    {file ? (
              ...
              {!file && !isPasteMode && (
                <div className='mt-8 pt-8 border-t border-slate-100 w-full'>
                  <button
                    ...
                    className='cursor-pointer flex items-center gap-2 text-sm font-bold text-[#074799] hover:underline mx-auto'
                  >
            ...
            <div className='w-full grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700'>
              ...
              <div className='lg:col-span-1 space-y-6'>
                <div className='bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm text-center'>
                  ...
                  <Button
                    onClick={resetAll}
                    className='w-full cursor-pointer bg-slate-900 text-white rounded-xl py-3 text-xs font-semibold hover:bg-slate-800'
                  >
                    Analyze New Resume
                  </Button>
                </div>
                ...
                <div className='bg-amber-50/50 border border-amber-100 p-8 rounded-[2rem] text-left'>
                  ...
                  <ul className='grid grid-cols-1 md:grid-cols-2 gap-3'>
                    {result?.improvements.map((imp, i) => (
                      <li
                        key={i}
                        className='text-xs text-slate-600 flex gap-2 items-start leading-relaxed'
                      >
